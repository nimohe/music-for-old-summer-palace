...
const VinylRecord: React.FC<VinylRecordProps> = ({ isPlaying, coverUrl }) => {
  return (
    <div className="relative flex flex-col items-center justify-center p-2 md:p-8">
      {/* Tonearm - 移动端适配 */}
      <div 
        className={`absolute top-0 right-[15%] md:right-1/4 z-20 transition-transform duration-700 ease-in-out origin-top-right`}
        style={{ transform: isPlaying ? 'rotate(15deg)' : 'rotate(-15deg)' }}
      >
        <div className="w-0.5 md:w-1 h-16 md:h-32 bg-gray-200 relative shadow-md rounded-full">
      ...
      {/* Decorative Interactive Controls - 移动端进一步缩小间距 */}
      <div className="mt-4 md:mt-12 flex items-center space-x-3 md:space-x-8">
        {[
          ...
          <button key={i} className="p-2 md:p-3 bg-gray-100/50 hover:bg-gray-100 rounded-full transition-colors group">
            <svg className="w-4 h-4 md:w-6 md:h-6 text-gray-600 group-hover:text-red-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
